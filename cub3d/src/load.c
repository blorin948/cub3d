...
/*   Updated: 2020/03/06 17:28:25 by blorin           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int		load(t_storage *ptr)
{
	if (!(ptr->text[0].img = mlx_xpm_file_to_image(ptr->mlx_ptr,
		ptr->texture.ouest, &ptr->text[0].x, &ptr->text[0].y)))
		return (error(ptr, "texture"));
	...
	if (!(ptr->text[2].img = mlx_xpm_file_to_image(ptr->mlx_ptr,
		ptr->texture.nord, &ptr->text[2].x, &ptr->text[2].y)))
		return (error(ptr, "texture"));
	load2(ptr);
	return (1);
}
...
void	ft_close(t_storage *ptr, int fd, char *line)
{
	int t;
